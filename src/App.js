...
const App = (props) => {
  ...
  const rotate180_deg = (Mat, l, r, u, d) => {
    ...
    for (i = u; i <= d; i++)
      for (j = l; j <= r; j++) Mat[i][j] = temp[i - u][j - l];
    return Mat;
  };
              ...
              onClick={() => {
                if (moveStr === "Shuffle")
                  if (!movement.motion) {
                    ...
                    for (let n = 1; n <= 20; n++)
                      shuffle_string +=
                        face[Math.floor(Math.random() * 6)].concat(
                          directions[Math.floor(Math.random() * 3)]
                        ) + " ";

                    async function Shuffler_Wait() {
                      //must be async func
                ...
                if (moveStr === "Solve")
                  if (!movement.motion) {
                    var solve_string = new CubeJS.initSolver()
                      .fromString(cubeStr)
                      .solve()
                      .replaceAll("'", "i");
                    console.log(
                      "Steps to solve 3x3 cube, from current cube state: ",
                      solve_string
                    );

                    async function Solver_Wait() {
                      //must be async func
                      alert("Steps to solve 3x3 cube, from current cube state: \n"+
                      solve_string+"\n\nAs per Herbert-Kociemba 2-phase algorithm, total moves required = "+solve_string.split(" ").length)
                    ...
                    cubeStr === defaultStr
                      ? alert(
                          "🧊 The 3x3 Cube is already in solved state !!! 😀👌"
                        )
                      : Solver_Wait();
                  }
